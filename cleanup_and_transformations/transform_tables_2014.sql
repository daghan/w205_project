...
DROP TABLE storms_2014_clean;
CREATE TABLE storms_2014_clean AS
SELECT  episode_id,
        end_day,
...
ALTER TABLE storms_2014_clean
SET SERDEPROPERTIES ('serialization.null.format' = '');


DROP TABLE storms_2014_clean_aggregate;
CREATE TABLE storms_2014_clean_aggregate AS
...
FROM storms_2014_clean
GROUP BY end_quarter, end_year, state
ORDER BY state, end_quarter,end_year;


DROP TABLE all_states_2014_clean;
CREATE TABLE all_states_2014_clean AS
...
FROM all_states_2014;
ALTER TABLE all_states_2014_clean
SET SERDEPROPERTIES ('serialization.null.format' = '');


select count(*) from storms_2014_clean_aggregate;
select count(*) from all_states_2014_clean;
